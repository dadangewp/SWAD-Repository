# -*- coding: utf-8 -*-
"""
Created on Mon Jul  6 14:40:08 2020
...
from keras.layers import LSTM, GRU, Lambda, RepeatVector, Activation, Dense, Dropout, Input, Embedding, add, multiply,Bidirectional, Flatten, concatenate
...
def RNN(X):
    ...
    aspect_repeat = RepeatVector(max_len)(aspect_embed)
    input_concat = concatenate([layer1, layer2], axis=-1)
    lstm, state_h, _ = LSTM(64, return_sequences=True, return_state=True)(input_concat)
    concat = concatenate([lstm, aspect_repeat], axis=-1)
    attend_weight = Attention()(concat)
    attend_weight_expand = Lambda(lambda x: K.expand_dims(x))(attend_weight)
    attend_hidden = multiply([lstm, attend_weight_expand])
    attend_hidden = Lambda(lambda x: K.sum(x, axis=1))(attend_hidden)
    
    attend_hidden_dense = Dense(32)(attend_hidden)
    last_hidden_dense = Dense(32)(state_h)
